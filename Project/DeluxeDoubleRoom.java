package Project;
//GROUP MEMBERS:
// FA20-BCS-017 (FAHAD ISHAQ)
//MINAHIL FATIMA (FA20-BCS-037)

import java.io.Serializable;

public class DeluxeDoubleRoom extends Room implements Serializable {
    protected static int noOfrooms = 10;
    public DeluxeDoubleRoom() {
        super();
    }
    public static String DeluxeDoubleRoomDetails() {
        return "NO. OF BEDS : 2 \nA.C Y/N : NO \nFREE DINNER : NO \nCHARGES PER NIGHT: 2500\n"+availabilty2();
    }
    public void setnumberofrooms() {
        noOfrooms = noOfrooms - 1;
    }
    public static String availabilty2(){
        return "NUMBER OF ROOMS AVAILABLE : "+noOfrooms;
    }

}
